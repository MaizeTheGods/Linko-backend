...
// Helper to safely trim values (avoid hidden spaces/quotes from copy/paste)
const trimVal = (v) => (typeof v === 'string' ? v.trim().replace(/^"|"$/g, '').replace(/^'|'$/g, '') : v);
...
let { CLOUDINARY_URL, CLOUDINARY_CLOUD_NAME, CLOUDINARY_API_KEY, CLOUDINARY_API_SECRET } = process.env;

// Trim potential whitespace/quotes
CLOUDINARY_URL = trimVal(CLOUDINARY_URL);
CLOUDINARY_CLOUD_NAME = trimVal(CLOUDINARY_CLOUD_NAME);
CLOUDINARY_API_KEY = trimVal(CLOUDINARY_API_KEY);
CLOUDINARY_API_SECRET = trimVal(CLOUDINARY_API_SECRET);

// Prefer CLOUDINARY_URL if present; otherwise, fall back to separate vars
if (CLOUDINARY_URL) {
